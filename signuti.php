...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Préparer la requête d'insertion sans 'prenom'
$sql = "INSERT INTO utilisateurs (nom, entreprise, email, mot_de_passe, departement, equipe, poste) VALUES (?, ?, ?, ?, ?, ?, ?)";

if ($stmt = $conn->prepare($sql)) {
    // Lier les paramètres sans 'prenom' - Assurez-vous que le nombre de paramètres et de variables correspond
    $stmt->bind_param("sssssss", $nom, $entreprise, $email, $mot_de_passe_hache, $departement, $equipe, $poste);

    if ($stmt->execute()) {
        // Stocker les informations de l'utilisateur dans la session
        $_SESSION['user_id'] = $conn->insert_id;  // ID de l'utilisateur inséré
        $_SESSION['nom'] = $nom;
